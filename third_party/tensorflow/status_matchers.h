...
namespace error {
inline void PrintTo(const absl::StatusCode code, std::ostream *os) {
  *os << absl::StatusCodeToString(code);
}
}  // namespace error
...
inline const absl::Status &GetStatus(const absl::Status &status) { return status; }
