...
std::list<Diff> diff_match_patch::diff_fromDelta(std::string_view text1,
                                                 std::string_view delta) {
  UnicodeEncoder unicode_encoder;
  return diff_fromDelta(unicode_encoder.from_bytes(std::string(text1)),
                        unicode_encoder.from_bytes(std::string(delta)));
}
...
std::size_t diff_match_patch::match_main(std::string_view text,
                                         std::string_view pattern,
                                         std::size_t loc) {
  UnicodeEncoder unicode_encoder;
  return match_main(unicode_encoder.from_bytes(std::string(text)),
                    unicode_encoder.from_bytes(std::string(pattern)), loc);
}
...
std::list<Patch> diff_match_patch::patch_make(std::string_view text1,
                                              std::string_view text2) {
  UnicodeEncoder unicode_encoder;
  return patch_make(unicode_encoder.from_bytes(std::string(text1)),
                    unicode_encoder.from_bytes(std::string(text2)));
}
...
std::list<Patch> diff_match_patch::patch_make(std::string_view text1,
  ...
  UnicodeEncoder unicode_encoder;
  return patch_make(unicode_encoder.from_bytes(std::string(text1)), diffs);
}
...
std::pair<std::string, std::vector<bool> > diff_match_patch::patch_apply(
    const std::list<Patch> &patches, std::string_view text) {
  UnicodeEncoder unicode_encoder;
  auto wide_result = patch_apply(patches, unicode_encoder.from_bytes(std::string(text)));
...
std::list<Patch> diff_match_patch::patch_fromText(std::string_view textline) {
  UnicodeEncoder unicode_encoder;
  return patch_fromText(unicode_encoder.from_bytes(std::string(textline)));
}
